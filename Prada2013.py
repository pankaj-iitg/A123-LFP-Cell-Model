...
def graphite_LGM50_ocp_Chen2020(sto):
    ...
    #### Added by Pankaj ####
    # return (
    #     1.1*1.9793 * np.exp(-39.3631 * sto)
    #     + (0.2482 - 0.05 - 0.05 + 0.03 +0.01)
    #     - 0.0909 * np.tanh(29.8538 * (sto - 0.1234))*0.5
    #     - 0.04478 * np.tanh(14.9159 * (sto - 0.2769))*0.4
    #     - 0.0205 * np.tanh(30.4444 * (sto - 0.6103))
    # )
    ...
    return (
        1.1*1.9793 * np.exp(-39.3631 * sto)
        + (0.2482) - 0.05 -0.05
        - 0.0909 * np.tanh(29.8538 * (sto - 0.1234))*0.3
        - 0.04478 * np.tanh(14.9159 * (sto - 0.2769))
        - 0.0205 * np.tanh(30.4444 * (sto - 0.6103))
    )
    ### Added by Pankaj ####
...
######## Added by Pankaj #### fixed
def graphite_LGM50_ocp_Chen2020_delithiation(sto):
    ### Added by Pankaj ####
    return (
        1.1*1.9793 * np.exp(-39.3631 * sto)
        + (0.2482) - 0.05 -0.05
...
######## Added by Pankaj #### fixed
def LFP_ocp_Afshar2017_lithiation(sto):
    #### Added by Pankaj ####
    ...
    c1 = (-150 * sto)
    c2 = -30 * (1 - sto)
    return 3.4077 - 0.020269 * sto + 0.6*0.5 * np.exp(c1) - 1.2*0.9 * np.exp(c2)
######## Added by Pankaj #### fixed
...
###fixed
def graphite_LGM50_ocp_Chen2020_lithiation(sto):
    ### Added by Pankaj ####
    return (
        0.75*1.9793 * np.exp(-39.3631 * sto)
        + (0.2482) - 0.05 + 0.01
        - 0.0909 * np.tanh(29.8538 * (sto - 0.1234))*0.3
        - 0.04478 * np.tanh(14.9159 * (sto - 0.2769))*0.7
        - 0.0205 * np.tanh(30.4444 * (sto - 0.6103))*0.15
    )
    ### Added by Pankaj ####
def LFP_ocp_Afshar2017_delithiation(sto):
    # # #### Added by Pankaj ####

    c1 = (-150 * sto)*0.2
    c2 = (-30 * (1 - sto))*2
    k = 3.4077*1.025 - 0.020269 * sto + 0.5*0.5 * np.exp(c1) - 0.5*0.9 * np.exp(c2)
...
######## Added by Pankaj 
def graphite_LGM50_ocp_Chen2020_delithiation_V2(sto):
    ### Added by Pankaj ####
    return (
        1.1*1.9793 * np.exp(-39.3631 * sto)
        + (0.2482) - 0.05 -0.05
...
######## Added by Pankaj
def LFP_ocp_Afshar2017_lithiation_V2(sto):
    #### Added by Pankaj ####
    ...
    c1 = (-150 * sto)
    c2 = -30 * (1 - sto)*5
    return 1.02*3.4077 - 0.020269 * sto + 0.6*0.5 * np.exp(c1) - 1.2*0.9 * np.exp(c2)
######## Added by Pankaj #### fixed


###not fixed
def graphite_LGM50_ocp_Chen2020_lithiation_V2(sto):
    ### Added by Pankaj ####
    return (
        0.75*1.9793 * np.exp(-39.3631 * sto)
        + (0.2482) - 0.05 + 0.01
    ...
    ### Added by Pankaj ####
def LFP_ocp_Afshar2017_delithiation_V2(sto):
    # # #### Added by Pankaj ####
...
###########   V2   ###################
    ...
    # """
    # Return lithiation OCP = delithiation OCP + A * tanh(B*(theta - C))
    # - A [V] : magnitude of hysteresis (~0.01-0.05 V typical)
    # - B      : sharpness (larger => more localized near C)
    # - C      : centre sto (0..1) where hysteresis is centred (approx plateau location)
    # """
    # A=0.01
    # B=10.0*10
    # C=0.5*100
    # dv=0.015*10
    # # ensure the delithiation function is called with the same mapping (theta) inside it
    # return LFP_ocp_Afshar2017_lithiation(sto) + A * np.tanh(B * (sto - C)) + dv
    
    
    
    
    # def LFP_ocp_lithiation_hysteresis(sto, A=0.01, B=10.0, C=0.5, dv=0.015):
    # """
    # Return lithiation OCP = delithiation OCP + A * tanh(B*(theta - C))
    # - A [V] : magnitude of hysteresis (~0.01-0.05 V typical)
    # - B      : sharpness (larger => more localized near C)
    ...
    # """
    # # ensure the delithiation function is called with the same mapping (theta) inside it
    # return LFP_ocp_Afshar2017(sto) + A * np.tanh(B * (sto - C)) + dv



# def LFP_ocp_lithiation_Pankaj(sto):
#     # c1 = -150 * sto*0.8
#     # c2 = -45* (1 - sto)*0.5
#     # k= 3.4310*0.98 - 0.009 * sto + 0.5*0.4687 * np.exp(c1) - 0.9*0.9 * np.exp(c2)
#     c1 = (-150 * sto*0.8)
#     c2 = (-45* (1 - sto)*0.7)
#     k= 3.4310*0.975 - 0.009 * sto + 0.4687 * np.exp(c1) - 0.9 * np.exp(c2)
#     return k

# def LFP_ocp_delithiation_Pankaj_corrected(sto):
#     c1 = -150 * sto #note Afsar thesis shows this coefficient to be 200. Remember this for future
#     c2 = ((-30 * (1 - sto)*0.8)*1.45)*50     ##1.5
#     k = 10*3.4077*1.04 - 0.020269 * sto + 0.5 * np.exp(c1) - 1.3*0.9 * np.exp(c2)
#     return k
